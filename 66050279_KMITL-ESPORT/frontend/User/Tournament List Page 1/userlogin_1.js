...
(function() {
            ...
            const matchSection = document.getElementById('upcoming-matches-section');
            ...
            async function loadData() {
                try {
                    ...
                    if (!tournaments || tournaments.length === 0) {
                        listContainer.innerHTML = '<p style="padding: 40px; text-align: center; color: #64748b;">No tournaments available in the database.</p>';
                    } else {
                        const now = new Date();
                        const openTournaments = tournaments.filter(t => !t.end_date || new Date(t.end_date) >= now);
                        if (openTournaments.length === 0) {
                            listContainer.innerHTML = '<p style="padding: 40px; text-align: center; color: #64748b;">No open tournaments available.</p>';
                        }
                        openTournaments.slice(0, 3).forEach(t => {
                            ...
                            const bannerSrc = t.tournament_banner || 'https://images.unsplash.com/photo-1542751371-adc38448a05e?w=800'; 

                            article.innerHTML = `
                                <div class="container-9">
                                    <img src="${bannerSrc}" onerror="this.src='https://images.unsplash.com/photo-1542751371-adc38448a05e?w=800'" style="width: 100%; height: 100%; object-fit: cover;">
                                </div>
                                <div class="container-10">
                                    <h2 class="text-wrapper-5">${t.tournament_name}</h2>
                                    <p class="major-tournament">${(t.tournament_type || '').split(' • ')[0]}</p>
                                    <div class="container-11">
                                        <div class="container-12">
                                            <img src="img/calendar-clock.svg" style="width: 16px; height: 16px;" />
                                            <span>2h remaining</span>
                                        </div>
                                        <div class="container-12">
                                            <img src="img/users.svg" style="width: 16px; height: 16px;" />
                                            <span>${t._count?.applications || 0} Teams</span>
                                        </div>
                                    </div>
                                    <button class="button-3" type="button" onclick="window.location.href='../Tournament Detail Page 2/Usertournament_2.html?id=${t.tournament_id}'">
                                        View
                                    </button>
                                </div>
                            `;
                    ...
                    // Matches rendering (Upgraded to match mockup)
                    matchListContainer.innerHTML = '';
                    let allMatches = [];
                    for (const t of tournaments) {
                        try {
                            const mRes = await fetch(`${API_BASE_URL}/tournaments/${t.tournament_id}/matches`);
                            if (mRes.ok) {
                                const matches = await mRes.json();
                                if (matches.length > 0) {
                                    matches.forEach(m => {
                                        allMatches.push({ ...m, tournament_name: t.tournament_name, tournament_match_date: t.match_date });
                                    });
                                }
                            }
                        } catch (e) {
                            console.warn('Failed to fetch matches for tournament', t.tournament_id);
                        }
                    }

                    let foundMatches = allMatches.length > 0;
                    if (foundMatches) {
                        // 1. Fetch ALL teams to have a banner map
                        ...
                        for (const tMatches of Object.values(tournamentGroups)) {
                            ...
                            // 2a. Fetch initial teams for seeds if needed
                            try {
                            ...
                            tMatches.forEach(m => {
                                ...
                                if (!m.team1_banner_url && m.team1_name) m.team1_banner_url = bannerMap[m.team1_name];
                                if (!m.team2_banner_url && m.team2_name) m.team2_banner_url = bannerMap[m.team2_name];
                            });
                        }
                        ...
                        allMatches.slice(0, 4).forEach(m => {
                            const banner1 = (m.team1_name && bannerMap[m.team1_name]) || '../../icons/user.svg';
                            const banner2 = (m.team2_name && bannerMap[m.team2_name]) || '../../icons/user.svg';
                            ...
                            row.innerHTML = `
                                <div class="match-meta-info">
                                    ...
                                    <span class="match-time-tag">${hasScore ? 'FINISHED' : `START IN <strong>${m.tournament_match_date ? new Date(m.tournament_match_date).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }) : 'TBD'}</strong>`}</span>
                                </div>
                    ...
                    matchSection.style.display = foundMatches ? 'flex' : 'none';
                } catch (error) {
                    console.error('Load Failed:', error);
                    listContainer.innerHTML = `<p style="padding: 40px; text-align: center; color: #ef4444;">Failed to connect to data source.</p>`;
                }
            }
            loadData();
        })();
