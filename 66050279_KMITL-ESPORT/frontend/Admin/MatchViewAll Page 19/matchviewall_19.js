            ...
            async function loadMatches() {
                try {
                    ...
                    for (const t of tournaments) {
                        try {
                            ...
                            if (mRes.ok) {
                                ...
                                // Include all matches that have teams, not just null scores
                                matches.forEach(m => {
                    ...
                    const getRoundLabel = (r) => {
                        if (!r) return 'R1';
                        if (r > 100) return 'Grand Finals';
                        if (r < 0) return `LB R${Math.abs(r)}`;
                        return `R${r}`;
                    };
                    ...
                    container.innerHTML = `<p style="padding:40px;text-align:center;color:#ef4444;">Failed to load data: ${err.message}</p>`;
                }
            }
            loadMatches();
        })();
